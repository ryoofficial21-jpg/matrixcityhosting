...
const newNameMatch = content.match(/New Name:\s*(.+)/i);
...
if (newNameMatch) {
    ...
    const embed = new EmbedBuilder()
        ...
        .addFields(
            ...
            {
                name: "New Name",
                value: newNameMatch[1].trim(),
                inline: true
            },
            {
                name: "Approved by",
                value: "⏳ Waiting for Staff Approval",
                inline: false
            }
        )
        .setFooter({
    ...
    const row = new ActionRowBuilder().addComponents(
        new ButtonBuilder()
            .setCustomId(`renameapprove_${message.author.id}`)
            .setLabel("✅ Approve Rename")
            .setStyle(ButtonStyle.Success),

        new ButtonBuilder()
            .setCustomId(`renamedeny_${message.author.id}`)
            .setLabel("❌ Deny")
            .setStyle(ButtonStyle.Danger)
    );
...
if (action === "renameapprove") {

    // ADMIN ONLY
     const isAdmin =
    interaction.member.roles.cache.has(config.rename.adminRoleId);

const isModerator =
    interaction.member.roles.cache.has(config.rename.moderatorRoleId);

if (!isAdmin && !isModerator) {
    return interaction.reply({
        content: "❌ Admin or Moderator only.",
        ephemeral: true
    });
}

const target = await interaction.guild.members.fetch(parts[1]);
...
const newName = fields[2].value.trim();

// kukunin yung prefix bago ang |
const prefixMatch = oldNickname.match(/^(.+?\|)\s*/);

let finalNickname;

if (prefixMatch) {
    finalNickname = `${prefixMatch[1]} ${newName}`;
} else {
    finalNickname = newName;
}

await target.setNickname(finalNickname).catch(() => {});

    const updatedEmbed = new EmbedBuilder()
        .setColor("Green")
        .setTitle("✅ Rename Request: Approved")
        .setThumbnail(config.images.thumbnail)
        .addFields(
            {
                name: "Requester",
...
if (action === "renamedeny") {
    ...
    const updatedEmbed = new EmbedBuilder()
        .setColor("Red")
        .setTitle("❌ Rename Request: Denied")
        .setThumbnail(config.images.thumbnail)
        .addFields(
            {
                name: "Requester",
